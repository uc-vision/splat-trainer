...
def add_resolvers():
    ...
    OmegaConf.register_new_resolver("const", lambda x: Constant(x))
    OmegaConf.register_new_resolver("linear", lambda x, y: Linear(x, y))
    OmegaConf.register_new_resolver("log_linear", lambda x, y: LogLinear(x, y))
    OmegaConf.register_new_resolver("piecewise", lambda init,values: Piecewise(init, values))
    OmegaConf.register_new_resolver("between", lambda t_start,t_end,varying: Between(t_start, t_end, varying))
