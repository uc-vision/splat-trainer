...
def main():
  ...
  def viewer_render_fn(camera_state: CameraState, img_wh: Tuple[int, int]):
    c2w = torch.from_numpy(camera_state.camera_t_world).to(train_config.device)
    projection = torch.tensor(camera_state.projection(img_wh), device=train_config.device)
