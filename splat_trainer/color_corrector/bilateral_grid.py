...
class BilateralGridInstance():
  ...
  def correct_for_train(self, rendering:Rendering, image_idx:int) -> Rendering:
  ...
  def correct_for_evaluation(self, rendering:Rendering, image:torch.Tensor) -> Rendering:
    cc_image = color_correct(rendering.image.unsqueeze(0), image.unsqueeze(0)).squeeze(0)
    rendering = replace(rendering, image=cc_image)
    return rendering


  def tvloss(self) -> float:
