...
class CameraRigTable(CameraTable):
  def __init__(self, rig_t_world:torch.Tensor,   # (N, 4, 4) - poses for the whole camera rig
    ...
    self.camera_poses = RigPoseTable(rig_t_world, camera_t_rig)
  ...
  def all_cameras(self):
    ...
    return torch.arange(0, n, device=self.camera_projection.device).view(self.shape())  
...
class MultiCameraTable(CameraTable):
  ...
  def shape(self) -> torch.Size:
    return torch.Size(self.camera_t_world.shape())
