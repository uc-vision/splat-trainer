...
class Trainer(Dispatcher):
  ...
  def evaluate(self):
    train_idx = self.camera_table.cameras.has_label(Label.Training)
    val_idx = self.camera_table.cameras.has_label(Label.Validation)
  ...
  def evaluate_batch_with(self, batch:List[ImageView], f:Callable[[int, Rendering], None]) -> dict:
    loss_metrics = []
  ...
  def training_step(self, batch:List[ImageView]) -> dict:
    ...
    loss_metrics = self.evaluate_batch_with(batch, f)
  ...
  def update_progress(self):
    ...
    if "train" in self.state_logger:
      ...
      desc.append(f"train(l1:{l1:.3f} ssim:{ssim:.3f} reg:{reg:.4f})")
